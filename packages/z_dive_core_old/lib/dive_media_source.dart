...
class DiveMediaSourceState {
  ...
  DiveMediaSourceState({this.currentTime = 0, this.duration = 0, this.mediaState = DiveMediaState.NONE});
...
class DiveMediaSource extends DiveTextureSource {
  ...
  DiveMediaSource({String name, this.localFile}) : super(inputType: DiveInputType.mediaSource, name: name) {
    _playbackTimer = Timer.periodic(Duration(milliseconds: 1000), timerCallback);
  }
  ...
  void timerCallback(Timer timer) {
    if (DiveCore.notifierFor(stateProvider).stateModel.mediaState == DiveMediaState.PLAYING) {
      _syncState();
    }
  }
