...
class DiveAudioMeterState {
  ...
  DiveAudioMeterState({
    ...
    required this.inputPeakHold,
    required this.magnitude,
    required this.magnitudeAttacked,
    required this.peak,
    required this.peakDecayed,
    required this.peakHold,
    required this.inputpPeakHoldLastUpdateTime,
    required this.peakHoldLastUpdateTime,
    this.lastUpdateTime,
    this.noSignal = true,
  });

  factory DiveAudioMeterState.empty() {
...
class DiveAudioMeterStateNotifier extends StateNotifier<DiveAudioMeterState> {
  ...
  DiveAudioMeterStateNotifier(DiveAudioMeterState? stateModel)
      : super(stateModel ?? DiveAudioMeterState.empty());
...
class DiveAudioMeterSource {
  ...
  Future<DiveAudioMeterSource?> create({required DiveSource source}) async {
    ...
    DiveCore.notifierFor(stateProvider).updateState(
        _clearDerived(currentState.copyWith(channelCount: channelCount)));
    return this;
  }
  ...
  DiveAudioMeterState _clearDerived(DiveAudioMeterState state) {
    // Get channel count or default to 1 channel.
    final channelCount = state.channelCount;
    return state.copyWith(
      noSignal: true,
      inputPeakHold: List.filled(channelCount, initialLevel),
      magnitudeAttacked: List.filled(channelCount, initialLevel),
      peakDecayed: List.filled(channelCount, initialLevel),
      peakHold: List.filled(channelCount, initialLevel),
      inputpPeakHoldLastUpdateTime: List.filled(channelCount, DateTime.now()),
      peakHoldLastUpdateTime: List.filled(channelCount, DateTime.now()),
    );
  }
