...
class _DivePositionEditState extends State<DivePositionEdit> {
  ...
  Widget build(BuildContext context) {
    ...
    final position = Padding(
        ...
        child: Row(
          ...
          children: [
            SizedBox(width: 60, child: Text('Position', textAlign: TextAlign.end)),
            Container(width: 8),
            Text('X:'),
            Container(width: 5),
            SizedBox(
    ...
    final buttons = Padding(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(width: 15),
          ElevatedButton(child: Text('Apply'), onPressed: widget.onApplyCallback == null ? null : _onApply),
        ]));
  ...
  void _useInitialState() {
    ...
    _scaleXCont.text = scale == null ? '' : (scale.x * 100.0).toStringAsFixed(1);
    _scaleYCont.text = scale == null ? '' : (scale.y * 100.0).toStringAsFixed(1);
  }
  ...
  void _onApply() {
    ...
    final pos = DiveVec2(double.parse(_posXCont.text), double.parse(_posYCont.text));
...
class _DiveMoveItemEditState extends State<DiveMoveItemEdit> {
  @override
  Widget build(BuildContext context) {
    final header = Padding(padding: EdgeInsets.only(top: 20), child: Text('Z-Priority'));
