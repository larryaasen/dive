    ...
    return MaterialApp(
        ...
        home: Scaffold(
          appBar: AppBar(
            ...
            actions: <Widget>[
              DiveStreamPlayButton(streamingOutput: _elements.state.streamingOutput),
              menuButton,
            ],
          ),
...
class _BodyWidgetState extends State<BodyWidget> {
      ...
      DiveInputs.video().forEach((videoInput) {
        ...
        DiveVideoSource.create(videoInput).then((source) {
          setState(() {
            ...
            _referencePanels.assignSource(source, _referencePanels.state.panels[panelIndex]);
            panelIndex++;
          });
  ...
  Widget buildApp(BuildContext context) {
    return BlocBuilder<DiveReferencePanelsCubit, DiveReferencePanels>(builder: (context, referencePanels) {
      // Build the card list from the assigned panels
      final cardList = referencePanels.panels
          .map((panel) => _widgetForSource(
              panel.assignedSource, BlocProvider.of<DiveReferencePanelsCubit>(context), panel))
          .toList();

      final aspectRatio = DiveCoreAspectRatio.HD.ratio;
      final sourceGrid = DiveGrid(aspectRatio: aspectRatio, children: cardList);

      final sources = Container(child: Wrap(spacing: 10, runSpacing: 0, children: <Widget>[sourceGrid]));

      final videoMix = Padding(
          padding: EdgeInsets.only(left: 1, right: 0),
          child: IntrinsicHeight(
              child: DivePreview(
                  _elements.state.videoMixes.length > 0 ? _elements.state.videoMixes[0].controller : null,
      ...
      final mainContent = Padding(
          ...
          child: Row(
            ...
            children: [Expanded(flex: 6, child: sources), Expanded(flex: 4, child: videoMix)],
          ));

      final body = Container(color: Theme.of(context).scaffoldBackgroundColor, child: mainContent);
      return body;
    });
  }
}
